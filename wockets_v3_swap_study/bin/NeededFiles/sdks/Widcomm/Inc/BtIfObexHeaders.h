...
public:
    ...
    tOBEX_ERRORS SetWriteTimeOut(UINT16 timeout);                       // added SDK 1.5, BTW-CE 1.5.0.2200
...
#if (_MSC_VER>0x500)
    // In later versions of Microsofts Visual Studio 2005 Beta 2+,
    // classes can no longer nominate private class members as friends.
    //
    // So we give CObexHeaders friend status to allow it access to GetBinaryPtr()
    friend class CObexHeaders;
#endif
};
...
public:
    ...
    tOBEX_ERRORS    SetLinkSupervisionTimeOut(UINT16 timeout);
    tOBEX_ERRORS    RegisterBinaryTarget(UINT8 scn, UINT8 *p_target, UINT16 target_len);        //added SDK 1.7, BTW-CE 1.7.0.1200
...
#if (_MSC_VER>0x0500)
    friend class CObexHeaders;  // See note in CObextClient
#endif
};
