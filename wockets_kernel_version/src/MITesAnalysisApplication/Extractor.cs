...
namespace MITesAnalysisApplication
{
    public class Extractor
    {
        ...
        public static void toARFFVanderbilt(string aDataDirectory, string masterDirectory, int maxControllers, string[] filter)
        {
            string[] file = null;
            string[] tokens = null;
            TextWriter vanderbiltCSV = null;
            TextReader vanderbiltReader = null;
            double vanderbiltUnixTime = 0;
            DateTime vanderbiltTime = new DateTime();
            Hashtable vanderbiltData = new Hashtable();
            bool vanderbiltFound = false;
            ...
            Hashtable currentEE = new Hashtable();
            string vanderbilt_line = "";



            file = Directory.GetFileSystemEntries(aDataDirectory + "\\" + OTHER_SUBDIRECTORY, "*ch.mat.csv");
            if (file.Length == 1)
            {
                vanderbiltReader = new StreamReader(file[0]);
                int month = 0;
                int day = 0;
                int year = 0;
                int hour = 0;        
               ...
               // string[] tokens;
                ...
                for (int j = 0; (j < 6); j++)
                {
                    vanderbilt_line = vanderbiltReader.ReadLine();//skip first 6 lines
                    if (vanderbilt_line.Contains("Date:"))
                    {
                        vanderbilt_line = vanderbilt_line.Substring(5, vanderbilt_line.Length - 5);
                        tokens = vanderbilt_line.Split(new char[] {'_'});
                        month = Convert.ToInt32(tokens[0]);
                        day = Convert.ToInt32(tokens[1]);
                        year = Convert.ToInt32(tokens[2]);
                    }

                    if (vanderbilt_line.Contains("Time:"))
                    {
                        vanderbilt_line = vanderbilt_line.Substring(5, vanderbilt_line.Length - 5);
                        tokens = vanderbilt_line.Split(new char[] { ':' });
                        hour = Convert.ToInt32(tokens[0]);
                        min = Convert.ToInt32(tokens[1]);
                        second = Convert.ToInt32(tokens[2]);
                    }
                    if (vanderbilt_line.Contains("Weight:"))
                    {
                        vanderbilt_line = vanderbilt_line.Substring(vanderbilt_line.IndexOf("Weight:")+7, vanderbilt_line.IndexOf(" kg") - (vanderbilt_line.IndexOf("Weight:")+7));
                        TotalMass = Convert.ToDouble(vanderbilt_line);
                    }
                }

                DateTime chamberTime;
                int chamberUnixTime;              
                chamberTime = new DateTime(year, month, day, hour, min, second);
                //chamberTime = chamberTime.AddHours(-1.0);
                while ((vanderbilt_line = vanderbiltReader.ReadLine())!=null)
                {                    
                    chamberTime=chamberTime.AddMinutes(1.0);
                    chamberUnixTime = (int)(UnixTime.GetUnixTime(chamberTime)/1000.0);

                   string chamberKey = chamberTime.Year + "-" + chamberTime.Month + "-" + chamberTime.Day + "-" + chamberTime.Hour + "-" + chamberTime.Minute;
  
                    tokens = vanderbilt_line.Split(new char[] { ' ', '\t',',' });
                    currentEE.Add(chamberKey, Convert.ToDouble(tokens[0]));
            ...
            TextWriter twCSV = new StreamWriter(aDataDirectory + "\\data.csv");
            tw.WriteLine("@RELATION wockets");
            //tw.WriteLine(Extractor.GetArffHeader());
            tw.WriteLine("@ATTRIBUTE TIME NUMERIC");
            twCSV.Write("ReadableTime,");
            twCSV.Write("RelativeTime,");
            tw.WriteLine("@ATTRIBUTE EE NUMERIC");
            twCSV.Write("EE-VO2(liter/min),");
            //tw.WriteLine("@ATTRIBUTE CALS NUMERIC");
            //twCSV.Write("Cals,");
            //tw.WriteLine("@ATTRIBUTE CUMCALS NUMERIC");
            //twCSV.Write("CumCals,");
            twCSV.Write(Extractor.GetArffHeaderOptionalCSVVanderbilt());
            //tw.WriteLine(Extractor.GetArffHeaderOptional());
            twCSV.WriteLine("activity");
            ...
            do
            {
                ...
                string mitesKey = dt.Year + "-" + dt.Month + "-" + dt.Day + "-" + dt.Hour + "-" + dt.Minute;
                //if (lastTimeStamp>=
                ...
                if ((Extractor.GenerateFeatureVector(lastTimeStamp)))
                {
                    ...
                    double cals = 0;
                    int currentSecond = (int)((lastTimeStamp - firstSecond)/1000.0);
                    if (currentEE.ContainsKey(mitesKey))
                    {
                        double ee = (double)currentEE[mitesKey];
                        if (ee > 0)
                        {
                            string arffSample = mitesKey + "," + currentSecond + "," + (double)currentEE[mitesKey] + "," + Extractor.toString() + "," + current_activity;
                            twCSV.WriteLine(arffSample);
                        }
                    }
        ...
        public static void toARFFOxycon(string aDataDirectory, string masterDirectory, int maxControllers, string[] filter)
        {
            string[] file =null;
            string[] tokens = null;
                            ...
                            while ((oxycon_line = oxyconReader.ReadLine()) != null)
                            {
                                ...
                                if (timeTokens.Length >= 2)
                                {
                                    ...
                                    if ((tokens[4].Length > 0) && (tokens[4] != "-"))
                                    {
                                        ...
                                        OxyconY.Add(Convert.ToDouble(tokens[5]));
                                    }
            ...
            TextWriter tw = new StreamWriter(aDataDirectory +"\\data.arff");
            //TextWriter twCSV = new StreamWriter(aDataDirectory + "\\data.csv");
            tw.WriteLine("@RELATION wockets");
            tw.WriteLine("@ATTRIBUTE TIME NUMERIC");
            //twCSV.Write("Time,");
            tw.WriteLine("@ATTRIBUTE V02-Kg NUMERIC");
            //twCSV.Write("V02-Kg,");
            //twCSV.Write(Extractor.GetArffHeaderOptionalCSV());
            tw.WriteLine(Extractor.GetArffHeaderOptional());
            //twCSV.WriteLine("activity");
            tw.Write("@ATTRIBUTE activity {unknown");
            Hashtable recorded_activities = new Hashtable();
            ...
            do
            {
                ...
                //if (current_activity.Equals("unknown") == false)
                //{
                if ((lastTimeStamp>0) && (Extractor.GenerateFeatureVector(lastTimeStamp)))
                {
                    
                    //lastTimeStamp -= correction;
                    if ((firstSecond == 0)&& (OxyconX.Count>3))
                    {                        
                    ...
                    if ((double)currentMilliSecond >= smallestOxyconValue)
                    {
                        ...
                        if (oxyconIndex > 5)
                        {
                            ...
                            if (prevEE != -1)
                            {
                                ...
                                ((Hashtable)modes[current_activity])[(int)currentEE] = (int)((Hashtable)modes[current_activity])[(int)currentEE] + 1;
                                string arffSample = currentMilliSecond + "," + currentEE.ToString("0.00") +  "," + Extractor.toString() + "," + current_activity;
                                //string csvSample = currentMilliSecond + "," + currentEE +  "," + Extractor.toString() + "," + current_activity;

                               // if (current_activity != "unknown")
                                //{
                                    tw.WriteLine(arffSample);
                                    //twCSV.WriteLine(csvSample);
                                //}
                            }
                        }
                        prevEE = currentEE;
...
*/
        ...
        static double WristMass = 0.0060;
        static double FootMass = 0.0145;
        ...
        //Vanderbilt, 1 Hip, 4 RightWrist, 7 LeftWrist, 8 RightAnkle, 11 LeftAnkle, 14 DominantUpperArm, 17 DominantUpperThigh
        static double[] limbs = new double[7] { TrunkMass, WristMass, WristMass, FootMass, FootMass,ArmMass, LegMass };
        static int[] limbsIndex = new int[7] {0, 1, 2,  3, 4,5,6 };
        static int[] limbsFV = new int[7] { 0, 1,2,3,4,5,6 };
...
*/
                    ...
                    if (((i + 1) % 3 == 0))
                    {
                        ...
                        if (ACSF)
                        {
                            ...
                            if (currenLimbIndex >= 0)
                            {
                                ...
                                fv[acSFIndex + limbsIndex[currenLimbIndex]] = sgForce;
        ...
        public static string GetArffHeaderOptionalCSVVanderbilt()
        {
            ...
            for (int i = 0; (i < inputRowSize); i++)
            {
                ...
                if (i % 3 == 1)
                    ...
                    axis_id = "Z";
                string sensorLocation = ((SXML.Sensor)sannotation.Sensors[sensor_id]).Location.Replace(' ', '_');
                sensorLocation = sensorLocation.Replace('_', '-');
                if (DCMean)
                    DCMean_ATTRIBUTES += "DCMean" + "_Sensor" + sensor_id + "_Location" + sensorLocation + "_Axis_" + axis_id + ",";
                if (DCArea)
                    DCArea_ATTRIBUTES += "DCArea" + "_Sensor" + sensor_id + "_Location" + sensorLocation + "_Axis_" + axis_id + ",";

                if (ACAbsMean)
                    ACAbsMean_ATTRIBUTES += "ACAbsMean" + "_Sensor" + sensor_id + "_Location" + sensorLocation + "_Axis_" + axis_id + ",";


                if (ACAbsArea)
                    ACAbsArea_ATTRIBUTES += "ACAbsArea" + "_Sensor" + sensor_id + "_Location" + sensorLocation + "_Axis_" + axis_id + ",";
                ...
                if (SpectralContent)
                {
                    ...
                    {
                        ACFFTFreqs_ATTRIBUTES += "ACFFTFreqs" + "_Sensor" + sensor_id + "_Location" + sensorLocation + "_Axis_" + axis_id + "_Max" + j + ",";

                        ACFFTMags_ATTRIBUTES += "ACFFTMags" + "_Sensor" + sensor_id + "_Location" + sensorLocation + "_Axis_" + axis_id + "_Max" + j + ",";
                    }
                }

                if (ACEntropy)
                    ACEntropy_ATTRIBUTES += "ACEntropy" + "_Sensor" + sensor_id + "_Location" + sensorLocation + "_Axis_" + axis_id + ",";


                if (ACEnergy)
                    ACEnergy_ATTRIBUTES += "ACEnergy" + "_Sensor" + sensor_id + "_Location" + sensorLocation + "_Axis_" + axis_id + ",";


                if (ACSkew)
                    ACSkew_ATTRIBUTES += "ACSkew" + "_Sensor" + sensor_id + "_Location" + sensorLocation + "_Axis_" + axis_id + ",";

                if (ACKur)
                    ACKur_ATTRIBUTES += "ACKur" + "_Sensor" + sensor_id + "_Location" + sensorLocation + "_Axis_" + axis_id + ",";

                if (ACQuartiles)
                {
                    for (int j = 0; (j < 3); j++)
                        ACQuartiles_ATTRIBUTES += "ACQuartiles" + "_Sensor" + sensor_id + "_Location" + sensorLocation + "_Axis_" + axis_id + "_Quartile" + j + ",";
                }

                if (ACVar)
                    ACVar_ATTRIBUTES += "ACVar" + "_Sensor" + sensor_id + "_Location" + sensorLocation + "_Axis_" + axis_id + ",";

                if (ACAbsCV)
                    ACAbsCV_ATTRIBUTES += "ACAbsCV" + "_Sensor" + sensor_id + "_Location" + sensorLocation + "_Axis_" + axis_id + ",";

                if (ACIQR)
                    ACIQR_ATTRIBUTES += "ACIQR" + "_Sensor" + sensor_id + "_Location" + sensorLocation + "_Axis_" + axis_id + ",";



                if (ACRange)
                    ACRange_ATTRIBUTES += "ACRange" + "_Sensor" + sensor_id + "_Location" + sensorLocation + "_Axis_" + axis_id + ",";

                if (ACBandEnergy)
                    ACBandEnergy_ATTRIBUTES += "ACBandEnergy" + "_Sensor" + sensor_id + "_Location" + sensorLocation + "_Axis_" + axis_id + ",";


                if (ACLowEnergy)
                    ACLowEnergy_ATTRIBUTES += "ACLowEnergy" + "_Sensor" + sensor_id + "_Location" + sensorLocation + "_Axis_" + axis_id + ",";

                if (ACModVigEnergy)
                    ACModVigEnergy_ATTRIBUTES += "ACModVigEnergy" + "_Sensor" + sensor_id + "_Location" + sensorLocation + "_Axis_" + axis_id + ",";

                if (ACPitch)
                    ACPitch_ATTRIBUTES += "ACPitch" + "_Sensor" + sensor_id + "_Location" + sensorLocation + "_Axis_" + axis_id + ",";

                if (ACMCR)
                    ACMCR_ATTRIBUTES += "ACMCR" + "_Sensor" + sensor_id + "_Location" + sensorLocation + "_Axis_" + axis_id + ",";
                ...
                if (ACCorr)
                {
                    ...
                    for (int k = i - 1; k >= 0; k--)
                    {
                        ...
                        ACCorr_ATTRIBUTES += "ACCorr" + "_Sensor" + sensor_id + "_Location" + sensorLocation + "_Axis_" + axis_id + "_" + "Sensor" + other_sensor_id + "_Location" + other_sensorLocation + "_Axis_" + other_sensor_axis + ",";
                    }
                }
            }
            ...
            {
                string sensorLocation = ((SXML.Sensor)sannotation.Sensors[i]).Location.Replace(' ', '_'); ;
                sensorLocation = sensorLocation.Replace('_', '-');
                if (DCAreaSensor)
                    DCAreaSensor_ATTRIBUTES += "DCArea_Sensor" + i + "_Location" + sensorLocation + ",";


                if (DCPostureDist)
                {
                    DCPostureDist_ATTRIBUTES += "DCPostureDist_Sensor" + i + "_Location" + sensorLocation + "_MeanX_MeanY,";
                    DCPostureDist_ATTRIBUTES += "DCPostureDist_Sensor" + i + "_Location" + sensorLocation + "_MeanX_MeanZ,";
                    DCPostureDist_ATTRIBUTES += "DCPostureDist_Sensor" + i + "_Location" + sensorLocation + "_MeanY_MeanZ,";
                }

                if (ACAbsAreaSensor)
                    ACAbsAreaSensor_ATTRIBUTES += "ACAbsArea_Sensor" + i + "_Location" + sensorLocation + ",";



                if (ACSVM)
                    ACSVM_ATTRIBUTES += "ACSVM_Sensor" + i + "_Location" + sensorLocation + ",";

                if (ACSF)
                    ACSF_ATTRIBUTES += "ACSF_Sensor" + i + "_Location" + sensorLocation + ",";

            }

            if (DCMeanTotal)
                DCMeanTotal_ATTRIBUTES = "DCMeanTotalAllSensors_LocationAll,";


            if (ACTotalAbsArea)
                ACTotalAbsArea_ATTRIBUTES = "ACTotalAbsAreaAllSensors_LocationAll,";


            if (ACTotalSVM)
                ACTotalSVM_ATTRIBUTES = "ACTotalSVMAllSensors_LocationAll,";


            if (ACTotalSF)
                ACTotalSF_ATTRIBUTES = "ACTotalSFAllSensors_LocationAll,";


            string allAttributes = "";
            //allAttributes = "MainMovement,";
        ...
        public static string GetArffHeaderOptionalCSV()
        {
...
//            if (fv[ACAbsAreaSensorIndex+0]<500)
            //if wrists are more than 1500 include them...
            //if all less than 500 only use the hip
            //
          
           /* s = "Hip";
            
            if ( (fv[ACAbsAreaSensorIndex + 2] > 1500) && (fv[ACAbsAreaSensorIndex + 5] > 1500) &&
                 (fv[ACAbsAreaSensorIndex + 6] > 1500))
                s += "Upper";

            if ((fv[ACAbsAreaSensorIndex + 1] > 1500) && (fv[ACAbsAreaSensorIndex + 3] > 1500) &&
                 (fv[ACAbsAreaSensorIndex + 4] > 1500))
                s += "Lower";
            s += ",";*/
