...
public final class WocketDecoder //extends Decoder 
{
    ...
    boolean bcFalg = false;
    
    //*************************************************Decode***************************************************************************    
	public void Decode(byte[] data, int end, WocketParam sr, String macID, int lastSeqNum, JTextArea textArea) {		
		...
		double distance = 25;
		double period = logDt.getTimeInMillis() - sr.prevTStamp;
		...
		while ((rawDataIndex != length)) {
			...
			if ((packetPosition == bytesToRead)) {// a full packet is received
				...
				if ((packetType == SensorDataType.UNCOMPRESSED_DATA_PDU) || (packetType == SensorDataType.COMPRESSED_DATA_PDU)) {
					...
					if (bcFalg){						
						if (period < 74900 ) {
							distance = (period / batchCount);
							//System.out.println("distance: "+ distance);
						}
						tempTS = (long)logDt.getTimeInMillis() - (long)(batchCount * distance);
						bcFalg = false;	
					}

					tempTS = tempTS + distance;
					timeStamp2 = timeStamp2 + 25;					
			    	Calendar temp = Calendar.getInstance();
			    	temp.setTimeInMillis((long)tempTS);
			    	saveRawData(temp, sr.id, x + "," + y + "," + z );	
			    	
			    	///bafEncoder.encodeAndSaveData(temp, (int) x, (int) y, (int) z, macID);
			    	bafEncoder.encodeAndSaveData(temp, (int) x, (int) y, (int) z, sr.id);
					...
					if (sec == -1)
						...
						saveMean(temp, sr.id, sumx/counter + "," + sumy/counter + "," + sumz/counter );
				...
				// -----------------------Decoding other packets-------------------------------------
					...
					DecodeResponce(responseType, bytesToRead, sr, macID, lastSeqNum, logDt, textArea);
				}
		...
		System.out.println("received " + (lastSN - lastSeqNum) + " summary data");
		sr.last_seqNumber = lastSN; 
		sr.prevTStamp = logDt.getTimeInMillis();	
		sr.batchCount = batchCount;
	}
    ...
    //*************************Decode Responce***************************************
    public void DecodeResponce (ResponseTypes responseType, int bytesToRead, WocketParam wp, String macID, int lastSeqNum, Calendar logDt, JTextArea textArea ){
    	...
    	switch (responseType){
            case BL_RSP:
                ...
                if (batteryLevel > 500){
                	...
                	textArea.append("Battery Level: "+ ((double)((batteryLevel - 570)*100 / (720 - 570))) +"%\n");
                    textArea.update(textArea.getGraphics());
                	if (batteryLevel < 600){
            ...
            case AC_RSP:
                ...
                if ( ((acSeqNum - lastSeqNum) >= 1) || (acc_count < lastSeqNum) || (lastSeqNum == -1) ) {                	
                	saveACData(acSeqNum, acCount, wp.id, macID, logDt);
                	lastSN = acSeqNum; 
                } 
                break;
    ...
    public void saveACData(int seqNum, int summaryData, String id, String macID, Calendar logDt)
	{		
    	...
    	String outputFilePath = "c:/test/" + dateFormat.format(date) +"/"+hour.format(date)+"/";
      ...
      //----------- json format -------------------   
		...
		String jsonFilename = getFileNameForCurrentHour(outputFilePath, logDt, id) + ".json";
      ...
      //-------------------CSV format------------------------
        String filename = getFileNameForCurrentHour(outputFilePath, logDt, id) + ".csv";
  ...
  //**********************getFileName ************************************
    private String getFileNameForCurrentHour(String path, Calendar time, final String ID){
		...
		else if (files[0].contains("json"))
			return path + files[0].substring(0, (files[0].length()-5));
		else 
			return path + files[0].substring(0, (files[0].length()-4));
	}
	...
	{		
    	...
    	SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd");
    	Calendar temp = Calendar.getInstance();
    	//temp.setTimeZone(TimeZone.getTimeZone("GMT-4"));
    	Date date = temp.getTime();
    	String outputFilePath = "c:/test/" + dateFormat.format(date) +"/control/";
    	File outputDir = new File(outputFilePath);
    	if(!outputDir.isDirectory()){
    		outputDir.mkdirs();
		}
        String filename = outputFilePath+"Wocket_"+id+"_1s-RawMean_Dominant-Ankle.csv";
        File f = new File(filename);
  ...
  //*****************************save Raw Data with corrected time stamps************************************
	...
	{	
    	...
    	String filename = outputFilePath+"Wocket_"+ id +"_RawData_Dominant-Ankle.csv";
        File f = new File(filename);
        PrintWriter out = null;
        try {
                if (!(f.exists())) {
