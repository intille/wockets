...
using AppUpdater.Utilities; //CompareFileWriteTime
using SMSLib;
...
namespace AppUpdater.Logging
{
    ...
    public class Logger
    {
        ...
        private static String uri = "http://participate.media.mit.edu/WeightControl/Monitoring/postLog.php"; 
        ...
        private static bool _isLoggingByDay = true;
        ...
        private static LogLevels _logLevel = LogLevels.DEBUG;
        #endregion
        ...
        {
            Logger.phoneNumber = phoneNumber;
        }

        #region LOGGING
        #region LOG ERROR
        public static void LogError(Exception ex, bool sendRemoteLog)
        {
            LogError(_defaultLogName, "", ex, "", sendRemoteLog);
        }
        public static void LogError(string message, bool sendRemoteLog)
        {
            LogError(_defaultLogName, "", message, "", sendRemoteLog);
        }
        public static void LogError(string codeLocation, Exception ex, bool sendRemoteLog)
        {
            LogError(_defaultLogName, codeLocation, ex, "", sendRemoteLog);
        }
        public static void LogError(string codeLocation, string message, bool sendRemoteLog)
        {
            LogError(_defaultLogName, codeLocation, message, "", sendRemoteLog);
        }
        public static void LogError(string codeLocation, Exception ex, string notes, bool sendRemoteLog)
        {
            LogError(_defaultLogName, codeLocation, ex, notes, sendRemoteLog);
        }
        public static void LogError(string codeLocation, string logevent, string notes, bool sendRemoteLog)
        {
            LogError(_defaultLogName, codeLocation, logevent, notes, sendRemoteLog);
        }
        public static void LogError(string logName, string codeLocation, string logevent, string notes, bool sendRemoteLog)
        {
            Log(logName, LogLevels.ERROR, codeLocation, logevent, notes, sendRemoteLog);
        }
        public static void LogError(string logName, string codeLocation, Exception ex, string notes, bool sendRemoteLog)
        {
            string message = ((ex != null) ? ex.ToString() : "");
            Log(logName, LogLevels.ERROR, codeLocation, message, notes, sendRemoteLog);
        }
        #endregion

        #region LOG WARNING
        public static void LogWarning(string message, bool sendRemoteLog)
        {
            LogWarning(_defaultLogName, "", message, "", sendRemoteLog);
        }
        public static void LogWarning(string codeLocation, string message, bool sendRemoteLog)
        {
            LogWarning(_defaultLogName, codeLocation, message, "", sendRemoteLog);
        }
        public static void LogWarning(string codeLocation, string logevent, string notes, bool sendRemoteLog)
        {
            LogWarning(_defaultLogName, codeLocation, logevent, notes, sendRemoteLog);
        }
        public static void LogWarning(string logName, string codeLocation, string logevent, string notes, bool sendRemoteLog)
        {
            Log(logName, LogLevels.WARNING, codeLocation, logevent, notes, sendRemoteLog);
        }
        #endregion

        #region LOG INFO
        public static void LogInfo(string message, bool sendRemoteLog)
        {
            LogInfo(_defaultLogName, "", message, "", sendRemoteLog);
        }
        public static void LogInfo(string codeLocation, string message, bool sendRemoteLog)
        {
            LogInfo(_defaultLogName, codeLocation, message, "", sendRemoteLog);
        }
        public static void LogInfo(string codeLocation, string logevent, string notes, bool sendRemoteLog)
        {
            LogInfo(_defaultLogName, codeLocation, logevent, notes, sendRemoteLog);
        }
        public static void LogInfo(string logName, string codeLocation, string logevent, string notes, bool sendRemoteLog)
        {
            Log(logName, LogLevels.INFO, codeLocation, logevent, notes, sendRemoteLog);
        }
        #endregion
        ...
        {
            Log(logName, LogLevels.DEBUG, codeLocation, logevent, notes, false);
        }
        #endregion

        public static void Log(string logname, LogLevels level, string codeLocation, string logevent, string notes, bool sendRemoteLog)
        {
            if (level >= _logLevel)
            {
                ...
                try
                {
                    ...
                    lock (typeof(Logger))
                    {
                        ...
                        // Send SMS when log entry is created.
                        if (sendRemoteLog)
                        {
                            if (HttpPostDataManager.IsIpAcquired)
                            {
                                Logger.LogDebug("Ip is acquired.");

                                // need a thread to do it because it's blocking. 
                                MessageToSend newMessage = new MessageToSend(uri, phoneNumber, logEntry);
                                Thread sendDataThread = new Thread(newMessage.startConnection);
                                sendDataThread.Start();

                                /*
                                if (!EmailSender.SendEmailThroughInternet("mitsmsmgr@gmail.com", "Status", logEntry))
                                {
                                    Logger.LogError("Pocket Outlook account not set. Using SMS instead", false);

                                    SMSManager smsMgr = new SMSManager("");
                                    smsMgr.sendData("mitsmsmgr@gmail.com", "Status", logEntry, "500");
                                }
                                */
                            ...
                            {
                                Logger.LogDebug("Ip is not acquired. ");
    ...
    {
        private String uri, clientNum, msgContent;
