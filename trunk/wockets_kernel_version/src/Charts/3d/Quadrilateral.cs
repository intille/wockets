...
namespace System.Drawing
{
    ...
    public class Quadrilateral : IDisposable
    {
        ...
        {
            GpPointF[] points= new GpPointF[4];
            m_path.GetPathPoints(points);
            if ((m_path.GetPointCount() == 0) || (points.Length == 0))
                return false;
        ...
        ///   <c>true</c> if the point is contained within the quadrilateral.
        /// </returns>
        public static bool Contains(GpPointF point, GpPointF[] cornerPoints)
        {
            int intersections = 0;
        ...
        /// </returns>
        private static bool DoesIntersect(GpPointF point, GpPointF point1, GpPointF point2)
        {
            float x2 = point2.X;
            float y2 = point2.Y;
            float x1 = point1.X;
        ...
        private static byte[] s_quadrilateralPointTypes = new byte[]  {   
                                                                          (byte)PathPointType.PathPointTypeStart,
                                                                          (byte)PathPointType.PathPointTypeLine,
                                                                          (byte)PathPointType.PathPointTypeLine,
                                                                          (byte)PathPointType.PathPointTypeLine | (byte)PathPointType.PathPointTypeCloseSubpath 
                                                                      };
