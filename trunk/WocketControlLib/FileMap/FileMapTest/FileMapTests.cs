     ...
     // [ Test ]
      public void TestNonBackedMMF()
      {
         try {
            ...
            Debug.Assert(false, "Write error: " + e);//Assertion.Fail("Write error: " + e);
         }
      }
      ...
      public void TestNamedMMF()
      {
         try {
            ...
            Debug.Assert(false, "Failed Named MMF: " + e);
         }
      }
      ...
      public void TestMMFFileWrite()
      {
         try {
            ...
            Debug.Assert(false, "Write error: " + e);
         }
      }
      ...
      public void TestMMFViewSeeking()
      {
         ...
         using ( Stream view = map.MapView(MapAccess.FileMapWrite, 0, MAX_BYTES) )
         {
            ...
            Debug.Assert(12 == (int)view.Position);//Assertion.AssertEquals(12, (int)view.Position);
            // Seek from current pos
            view.Seek(1, SeekOrigin.Current);
            Debug.Assert(13 == (int)view.Position);//Assertion.AssertEquals(13, (int)view.Position);
            // seek from end
            view.Seek(-1, SeekOrigin.End);
            Debug.Assert(MAX_BYTES - 1 == (int)view.Position);//Assertion.AssertEquals(MAX_BYTES-1, (int)view.Position);

            try {
               // no seeking past end of stream
               view.Seek(1, SeekOrigin.End);
               Debug.Assert(false, "Seeked past end of stream");
      ...
      public void TestMMFViewSize()
      {
         ...
         using ( Stream view = map.MapView(MapAccess.FileMapWrite, 0, MAX_BYTES) )
         {
            ...
            view.WriteByte(0x01);
            Debug.Assert(MAX_BYTES == (int)view.Position);//Assertion.AssertEquals(MAX_BYTES, (int)view.Position);
            // no reading past end of stream
            Debug.Assert(-1 == view.ReadByte());//Assertion.AssertEquals(-1, view.ReadByte());
         }
         map.Close();
      }
      ...
      private void VerifyBytesInStream(Stream stream, int count)
      {
         for ( int i=0; i < count; i++ ) {
            ...
            if ( b == -1 )
               Debug.Assert(false, "Reached end of stream prematurely");
           Debug.Assert((byte)(i / 255) == (byte)b);//Assertion.AssertEquals((byte)(i/255), (byte)b);
         }
      }
