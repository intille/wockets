...
namespace System.Drawing.PieChart
{
    ...
    public class PieSlice : IDisposable, ICloneable
    {
        ...
        /// </returns>
        public bool Contains(GpPointF point)
        {
        ...
        ///   <c>Graphics</c> used to draw the pie slice.
        /// </param>
        internal void DrawVisiblePeriphery(GraphicsPlus graphics)
        {
        ...
        ///   <c>Graphics</c> used to draw the pie slice.
        /// </param>
        internal void DrawBottom(GraphicsPlus graphics)
        {
        ...
        /// </returns>
        internal bool PieSliceContainsPoint(GpPointF point)
        {
        ...
        /// </returns>
        internal bool PeripheryContainsPoint(GpPointF point)
        {
            PeripherySurfaceBounds[] peripherySurfaceBounds = GetVisiblePeripherySurfaceBounds();
            foreach (PeripherySurfaceBounds surfaceBounds in peripherySurfaceBounds)
            {
        ...
        /// </returns>
        internal bool StartSideContainsPoint(GpPointF point)
        {
            if (m_sliceHeight > 0)
        ...
        /// </returns>
        internal bool EndSideContainsPoint(GpPointF point)
        {
            if (m_sliceHeight > 0)
        ...
        /// </returns>
        internal bool BottomSurfaceSectionContainsPoint(GpPointF point)
        {
            if (m_sliceHeight > 0)
            {
        ...
        ///   <c>Brush</c> object.
        /// </returns>
        protected virtual BrushPlus CreateBrushForSide(Color color, double angle)
        {
            return new SolidBrushPlus(ColorUtil.CreateColorWithCorrectedLightness(color, -(float)(ColorUtil.BrightnessEnhancementFactor1 * (1 - 0.8 * Math.Cos(angle * Math.PI / 180)))));
        }
        ...
        ///   <c>Brush</c> object.
        /// </returns>
        protected virtual BrushPlus CreateBrushForPeriphery(Color color)
        {
            ...
            LinearGradientBrush brush = new LinearGradientBrush(new GpRectF(m_boundingRectangle.X,m_boundingRectangle.Y,m_boundingRectangle.Width,m_boundingRectangle.Height), Color.Blue, Color.White, LinearGradientMode.LinearGradientModeHorizontal);

            brush.SetInterpolationColors(new Color[] { 
                                                ColorUtil.CreateColorWithCorrectedLightness(color, -ColorUtil.BrightnessEnhancementFactor1 / 2), 
                                                color, 
                                                ColorUtil.CreateColorWithCorrectedLightness(color, -ColorUtil.BrightnessEnhancementFactor1), 
            }, new float[] { 0F, 0.1F, 1.0F }); 
        ...
        private PeripherySurfaceBounds[] GetVisiblePeripherySurfaceBounds()
        {
            ...
            if (!(m_sweepAngle == 0 || (m_startAngle >= 180 && m_startAngle + m_sweepAngle <= 360)))
            {
                ...
                if (StartAngle < 180)
                {
                    ...
                    GpPointF x1 = new GpPointF(m_pointStart.X, m_pointStart.Y);
                    float fi2 = EndAngle;
                    GpPointF x2 = new GpPointF(m_pointEnd.X, m_pointEnd.Y);
                ...
                {
                    float fi1 = 0;
                    GpPointF x1 = new GpPointF(m_boundingRectangle.Right, m_center.Y);
                    float fi2 = EndAngle;
                    GpPointF x2 = new GpPointF(m_pointEnd.X, m_pointEnd.Y);
                    if (fi2 > 180)
                    {
        ...
        private PeripherySurfaceBounds[] GetHiddenPeripherySurfaceBounds()
        {
            ...
            if (!(m_sweepAngle == 0 || (m_startAngle >= 0 && m_startAngle + m_sweepAngle <= 180)))
            {
                ...
                if (m_startAngle + m_sweepAngle > 180)
                {
                    ...
                    {
                        fi1 = 180;
                        x1 = new GpPointF(m_boundingRectangle.Left, m_center.Y);
                        fi2 = EndAngle;
                        x2 = new GpPointF(m_pointEnd.X, m_pointEnd.Y);
                        peripherySurfaceBounds.Add(new PeripherySurfaceBounds(fi1, fi2, x1, x2));
                    }
                }
            }
        ...
        /// </returns>
        private bool CylinderSurfaceSectionContainsPoint(GpPointF point, float startAngle, float endAngle, GpPointF point1, GpPointF point2)
        {
            if (m_sliceHeight > 0)
            {
                return Quadrilateral.Contains(point, new GpPointF[] { point1, new GpPointF(point1.X, point1.Y + m_sliceHeight), new GpPointF(point2.X, point2.Y + m_sliceHeight), point2 });
            }
            return false;
        }
        ...
        /// </returns>
        private bool PieSliceContainsPoint(GpPointF point, float xBoundingRectangle, float yBoundingRectangle, float widthBoundingRectangle, float heightBoundingRectangle, float startAngle, float sweepAngle)
        {
        ...
        {
            public PeripherySurfaceBounds(float startAngle, float endAngle, GpPointF startPoint, GpPointF endPoint)
            {
            ...
            public GpPointF StartPoint
            {
                get { return m_startPoint; }
            }

            public GpPointF EndPoint
            {
            ...
            private GpPointF m_startPoint;
            private GpPointF m_endPoint;
        }
