...
namespace weka.core
{
	...
	/// <summary> Implementation of Active-sets method with BFGS update
	...
	{
		/// <summary> Get the minimal function value
		/// 
		/// </summary>
		/// <returns> minimal function value found
		/// </returns>
		virtual public double MinFunction
		{
			get
			{
				return m_f;
		...
		/// <summary> Set the maximal number of iterations in searching (Default 200)
		/// 
		/// </summary>
		/// <param name="it">the maximal number of iterations
		/// </param>
		...
		/// <summary> Set whether in debug mode
		/// 
		/// </summary>
		/// <param name="db">use debug or not
		/// </param>
		...
		/// <summary> Get the variable values.  Only needed when iterations exceeds 
		/// the max threshold.
		/// 
		/// </summary>
		/// <returns> the current variable values
		/// </returns>
		virtual public double[] VarbValues
		{
			get
			{
				return m_X;
		...
		protected internal double m_ALF = 1.0e-4;
		
		protected internal double m_BETA = 0.9;
		
		protected internal double m_TOLX = 1.0e-6;
		
		protected internal double m_STPMX = 100.0;
		
		protected internal int m_MAXITS = 200;
		
		protected internal static bool m_Debug = false;
		
		// function value
		protected internal double m_f;
		...
		protected internal static double m_Epsilon;
		protected internal static double m_Zero;
		...
		*/
		protected internal virtual double[] evaluateHessian(double[] x, int index)
		{
			return null;
		}
...
iterates: 
		...
		/// <summary> Solve the linear equation of TX=B where T is a triangle matrix
		/// It can be solved using back/forward substitution, with O(N^2) 
		/// complexity
		/// </summary>
		/// <param name="t">the matrix T
		/// </param>
		/// <param name="b">the vector B
		/// </param>
		/// <param name="isLower">whether T is a lower or higher triangle matrix
		/// </param>
		/// <param name="isZero">which row(s) of T are not used when solving the equation. 
		/// If it's null or all 'false', then every row is used.
		/// </param>
		/// <returns> the solution of X
		/// </returns>
		public static double[] solveTriangle(Matrix t, double[] b, bool isLower, bool[] isZero)
		{
			int n = b.Length;
			...
			if (isLower)
			{
				...
				if (j < n)
				{
					result[j] = b[j] / t.getXmlElement(j, j);
					...
					for (; j < n; j++)
					{
						if (!isZero[j])
						{
							...
							for (int k = 0; k < j; k++)
								numerator -= t.getXmlElement(j, k) * result[k];
							result[j] = numerator / t.getXmlElement(j, j);
						}
						else
							result[j] = 0.0;
					}
				...
				if (j >= 0)
				{
					...
					for (; j >= 0; j--)
					{
						if (!isZero[j])
						{
							double numerator = b[j];
							for (int k = j + 1; k < n; k++)
								numerator -= t.getXmlElement(k, j) * result[k];
							result[j] = numerator / t.getXmlElement(j, j);
						}
						else
							result[j] = 0.0;
		...
		/// <summary> One rank update of the Cholesky factorization of B matrix in BFGS updates,
		...
		/// 
		/// </summary>
		/// <param name="L">the unit triangle matrix L
		/// </param>
		/// <param name="D">the diagonal matrix D
		/// </param>
		/// <param name="v">the update vector v
		/// </param>
		/// <param name="coeff">the coeffcient of update
		/// </param>
		/// <param name="isFixed">which variables are not to be updated
		/// </param>
		protected internal virtual void  updateCholeskyFactor(Matrix L, double[] D, double[] v, double coeff, bool[] isFixed)
		{
			double t, p, b;
			int n = v.Length;
			...
			if (coeff > 0.0)
			{
				...
				for (int j = 0; j < n; j++)
				{
					...
					for (int r = j + 1; r < n; r++)
					{
						if (!isFixed[r])
						{
							double l = L.getXmlElement(r, j);
							vp[r] -= p * l;
							L.setXmlElement(r, j, l + b * vp[r]);
						}
						else
							L.setXmlElement(r, j, 0.0);
					}
				}
			}
			else
			{
				...
				sqrt = (sqrt < 0.0)?0.0:System.Math.Sqrt(sqrt);
				...
				for (int j = 0; j < n; j++)
				{
					...
					if (System.Double.IsNaN(D[j]))
					{
						throw new System.Exception("d[" + j + "] NaN! P=" + P[j] + ",d=" + d + ",t=" + t + ",p=" + p + ",sigma=" + sigma + ",sclar=" + coeff);
					}
					...
					if ((j < n - 1) && (System.Double.IsNaN(sigma) || System.Double.IsInfinity(sigma)))
						throw new System.Exception("sigma NaN/Inf! rho=" + rho + ",theta=" + theta + ",P[" + j + "]=" + P[j] + ",p=" + p + ",d=" + d + ",t=" + t + ",oldsigma=" + sigmaOld);
		...
		//UPGRADE_NOTE: Field 'EnclosingInstance' was added to class 'DynamicIntArray' to access its enclosing instance. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1019'"
		/// <summary> Implements a simple dynamic array for ints.</summary>
		//UPGRADE_NOTE: The access modifier for this class or class field has been changed in order to prevent compilation errors due to the visibility level. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1296'"
		public class DynamicIntArray
		{
			private void  InitBlock(Optimization enclosingInstance)
			{
				this.enclosingInstance = enclosingInstance;
			}
			private Optimization enclosingInstance;
			public Optimization Enclosing_Instance
			{
				get
				{
			...
			/// </param>
			public DynamicIntArray(Optimization enclosingInstance, int capacity)
			{
			...
			/// <summary> Produces a copy of this vector.
			/// 
			/// </summary>
			...
			public System.Object copy()
			{
				...
				DynamicIntArray copy = new DynamicIntArray(enclosingInstance, m_Objects.Length);
		...
		{
			{
				...
				m_Epsilon *= 2.0;
				m_Zero = System.Math.Sqrt(m_Epsilon);
				if (m_Debug)
					System.Console.Error.Write("Machine precision is " + m_Epsilon + " and zero set to " + m_Zero);
			}
		}
	}
}