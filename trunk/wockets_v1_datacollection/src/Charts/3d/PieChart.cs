...
namespace System.Drawing.PieChart
{
    ...
    public class PieChart3D : IDisposable
    {
            ...
            using (Brush fontBrush = new SolidBrush(m_foreColor))
            {
                foreach (PieSlice slice in m_pieSlices)
                {
                    if (slice.Text != null && slice.Text.Length > 0)
                    {
                        ...
                        graphics.DrawString(slice.Text, m_font, fontBrush, point.X,point.Y, drawFormat);
                    }
                }
            }
        }
        ...
        protected RectangleF GetFittingRectangle()
        {
            ...
            {
                boundingRectangle = Charts.RectangleFUtils.Union(boundingRectangle, m_pieSlices[i].GetFittingRectangle());
            }
        ...
        ///   <c>Graphics</c> used for drawing.
        /// </param>
        protected void DrawBottoms(GraphicsPlus graphics)
        {
            foreach (PieSlice slice in m_pieSlices)
            {
        ...
        /// </summary>
        protected Font m_font = new Font("Arial", 8,FontStyle.Regular);
        /// <summary>
