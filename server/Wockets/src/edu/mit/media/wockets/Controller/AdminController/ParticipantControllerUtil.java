...
public class ParticipantControllerUtil {
	...
	public static JSONObject getAssignedPhone(int pId)
	{
		...
		{	
			...
			if(objectList[6].toString().equals("1"))
				rowData.add("Active");
			if(objectList[6].toString().equals("0"))
				rowData.add("Inactive");
			if(objectList[6].toString().equals("2"))
				rowData.add("Pending");
	...
	public static void assignNewPhone(int pId,HttpServletRequest request)
	{
		Session session = HibernateSession.getSession();
		/***********************
		 * make new assigned phone status as Pending
		 * wait for acknowledgement from participant 
		 * set reason of change to current active phone
		 **************************/
		...
		SQLQuery q = session.createSQLQuery("UPDATE PARTICIPANT_PHONE SET Reason_of_Change =:reason WHERE Active_Status ='1' AND Participant_Id=:pId");
		//q.setCharacter("char",'0');
	...
	public static void assignNewSim(int pId,HttpServletRequest request)
	{
		Session session = HibernateSession.getSession();
		/***********************
		 * make all phoneNumber as inactive 
		 * because participant can have only one active phone number at a time
		 **************************/
